#!/usr/bin/env node
// install.mjs が入れた HeadLenss スキルを取り除く。
//
// 配置先は install.mjs と同じ 2 つ (両方から消す)。
//   - Claude Code: ~/.claude/skills/
//   - Codex:       $CODEX_HOME/skills/  (未設定なら ~/.codex/skills/)
...
//
// 削除対象はリポジトリの現在のファイル一覧ではなく、配置先の走査で決める。
...
/** 配置先の一覧 (install.mjs の skillTargets と同じ規則)。 */
function skillDirs() {
  const codexHome = (process.env.CODEX_HOME || '').trim() || resolve(homedir(), '.codex');
  const all = [resolve(homedir(), '.claude', 'skills'), resolve(codexHome, 'skills')];
  return [...new Set(all)];
}
...
function uninstall() {
  ...
  if (removed === 0) {
    if (scanned > 0) console.log(`No HeadLenss skills found in ${dirs.join(', ')}; nothing to remove.`);
    return;
  }
  console.log('Restart Claude Code / Codex so the removed skills disappear.');
}
